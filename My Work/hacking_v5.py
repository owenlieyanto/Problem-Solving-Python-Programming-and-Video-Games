...
attempts_left = 4
header = ['DEBUG MODE', f'{attempts_left} ATTEMPT(S) LEFT', '']
...
while guess != password and attempts_left > 0:
    ...
    window.draw_string(f'{attempts_left}', 0, string_height)

    #           check warning
    #               if attempts left equals 1
    if attempts_left == 1:
        #               display warning
        warning_msg = '*** LOCKOUT WARNING ***'
        warning_posX = window.get_width() - window.get_string_width(warning_msg)
        warning_posY = window.get_height() - string_height
        window.draw_string(warning_msg, warning_posX, warning_posY)
